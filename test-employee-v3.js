...
(async () => {
  ...
  console.log(await redeemBtns.first().isVisible() ? '✅ 兑换 buttons' : '❌ 兑换 buttons');
  ...
  if (await redeemBtn.isVisible()) {
    ...
    for (const t of ['请确认兑换', '商品信息', '积分明细', '应付积分', '当前积分余额', '确认兑换', '收货信息', '温馨提示', '数量']) {
      const el = page.locator(`text=${t}`).first();
      const v = await el.isVisible({ timeout: 2000 }).catch(() => false);
      console.log(v ? `✅ ${t}` : `❌ ${t}`);
  ...
  for (const t of ['全部', '待发货', '已发货', '已完成', '已取消']) {
    const el = page.locator(`text=${t}`).first();
    const v = await el.isVisible({ timeout: 2000 }).catch(() => false);
  ...
  if (await detailBtn.isVisible({ timeout: 2000 }).catch(() => false)) {
    ...
    for (const t of ['订单详情', '订单状态', '商品信息', '积分明细', '实付积分', '收货信息', '订单信息', '返回订单列表', '联系客服']) {
      const el = page.locator(`text=${t}`).first();
      const v = await el.isVisible({ timeout: 2000 }).catch(() => false);
      console.log(v ? `✅ ${t}` : `❌ ${t}`);
  ...
  for (const t of ['我的可用积分', '累计获得', '累计使用', '兑换次数', '积分商城', '兑换记录', '积分规则', '帮助中心', '积分获取途径', '工龄积分', '绩效奖励', '节日福利', '特别贡献', '积分明细']) {
    const el = page.locator(`text=${t}`).first();
    const v = await el.isVisible({ timeout: 2000 }).catch(() => false);
    console.log(v ? `✅ ${t}` : `❌ ${t}`);
